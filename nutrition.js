      ...
      async function saveLog() {
        ...
        if (isLoggedIn()) {
          ...
          localStorage.setItem(
            HISTORY_KEY,
            JSON.stringify(filtered.slice(0, 7)),
          );
        }
        showToast("今日記錄已儲存 ✓");
        renderHistory();
    ...
    <div class="food-item">
      ...
      function filterFoods() {
        ...
        renderFoodList(
          q
            ? FOOD_DB.filter(
                (f) => f.name.includes(q) || f.en.toLowerCase().includes(q),
              )
            : FOOD_DB,
        );
      }
    ...
    <div class="log-item">
      ...
      function updateChart() {
        ...
        document.getElementById("legend-fat").textContent =
          `${Math.round(t.fat)}g`;
        const pct =
          tdeeTarget > 0 ? Math.min((t.cal / tdeeTarget) * 100, 100) : 0;
      ...
      function calcTDEE() {
        ...
        const activity = parseFloat(
          document.getElementById("tdee-activity").value,
        );
